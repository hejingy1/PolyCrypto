...
def policy(new_illiqu, new_value, new_row, past_data):
    ...
    if check_time_interval(new_row):
        if new_illiqu <= past_data.illiquidity_average_morning:
            if new_value <= past_data.volume_average_morning:
                return 3
            return 1
            
        elif new_value <= past_data.volume_average_morning:
            return 0
        return 0
    else:
        if new_illiqu <= past_data.illiquidity_average_night:
            if new_value <= past_data.volume_average_night:
                return 3
            return 1

        elif new_value <= past_data.volume_average_night:
            return 0
        return 0
...
class Pastdata:
    ...
    def fetch_past_data(self, type):
        ...
        # for i in self.past_data["n"]:
        #     self.past_volume_total.append(i)
        #self.past_data[["n"]] = StandardScaler().fit_transform(self.past_data[["n"]])
